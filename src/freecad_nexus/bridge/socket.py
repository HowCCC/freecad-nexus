...
- Reconnect for subsequent requests without replaying uncertain executions
"""
...
from freecad_nexus.bridge.base import (
    ConnectionStatus,
    DocumentInfo,
    ExecutionResult,
    FreecadBridge,
...
SOCKET_RESPONSE_LIMIT = 64 * 1024 * 1024
LEGACY_REQUEST_LIMIT = 60 * 1024


def socket_python_source(code: str) -> str:
    """Fit large runtime scripts within the legacy bridge's 64 KiB line limit."""
    if len(json.dumps(code).encode("utf-8")) < LEGACY_REQUEST_LIMIT:
        return code
    ...
    wrapped = (
        ...
        + repr(encoded)
        + ")).decode('utf-8'), '<mcp-python>', 'exec'), globals(), globals())"
    )
    if len(json.dumps(wrapped).encode("utf-8")) >= LEGACY_REQUEST_LIMIT:
        raise ValueError(
            "Compressed Python exceeds the legacy socket request limit; use XML-RPC for this payload"
        )
    return wrapped
...
class SocketBridge(FreecadBridge):
    ...
    async def _open_connection(self) -> None:
        """Open/ping a socket while holding only the connection lock."""
        try:
            if self._writer is not None:
                self._writer.close()
            self._reader, self._writer = await asyncio.wait_for(
                asyncio.open_connection(
                    self._host, self._port, limit=SOCKET_RESPONSE_LIMIT
                ),
    ...
    async def _send_request(
        ...
        if not self._connected and self._auto_reconnect:
            # Only a new caller's request may reconnect. Never replay a request
            # after a transport failure where execution outcome is uncertain.
            await self.connect()
            ...
            try:
                if not self._connected or self._writer is None or self._reader is None:
                    raise ConnectionError(
                        "Socket connection is closed; reconnect before the next request"
                    )
                # Send request
                request_data = json.dumps(request).encode("utf-8") + b"\n"
                if len(request_data) > LEGACY_REQUEST_LIMIT + 1024:
                    raise ValueError("Request exceeds legacy socket line limit")
                ...
                if not isinstance(response, dict) or response.get("id") != request_id:
                    raise ConnectionError(
                        "Socket response ID does not match the request"
                    )
                ...
                # A late reply must never be read as the following call's result.
                self._connected = False
                self._writer.close()
                raise
                ...
                self._connected = False
                self._writer.close()
                # The request may already have mutated FreeCAD. Never replay it,
                # and never reconnect/ping while holding this same request lock.
                msg = f"Connection lost; execution outcome is unknown and was not retried: {e}"
                raise ConnectionError(msg) from e
    ...
    async def execute_python(
        ...
        try:
            code = socket_python_source(code)
