"""FreeCAD Nexus resources for exposing FreeCAD state.
...
def register_resources(mcp: Any, get_bridge: Any) -> None:
    """Register FreeCAD resources with the FreeCAD Nexus.

    Args:
        mcp: The FastMCP (FreeCAD Nexus) instance.
...
3. Create new document if starting fresh work""",
    ...
    async def resource_capabilities() -> str:
        ...
        is available when working with the FreeCAD Nexus.
        ...
        """
        capabilities = {
            "description": "FreeCAD Nexus - Control FreeCAD via Model Context Protocol",
            "tools": {
                "execution": {
                    ...
                    "tools": [
                        ...
                        },
                        {
                            ...
                            "description": "Get FreeCAD Nexus environment info (instance_id, OS, hostname, FreeCAD connection)",
                            "key_params": [],
                        },
                    ],
                },
                ...
                "assembly": {
                    "description": "FreeCAD 1.1 Assembly workbench",
                    "tools": [
                        {"name": n, "description": d} for n, d in [
                            ("assembly_create", "Create native Assembly::AssemblyObject"),
                            ("assembly_insert_component", "Insert AssemblyLink or App::Link component"),
                            ("assembly_insert_external_component", "Open and insert a component from an external FreeCAD file"),
                            ("assembly_remove_component", "Remove a component and dependent joints"),
                            ("assembly_create_part", "Create a native Assembly part and body"),
                            ("assembly_set_placement", "Set component placement"),
                            ("assembly_add_constraint", "Create and reference a native joint"),
                            ("assembly_ground_component", "Ground a component"),
                            ("assembly_solve", "Run the Assembly solver"),
                            ("assembly_instance_sync_status", "Inspect flexible instance joint parameter differences"),
                            ("assembly_sync_instances", "Synchronize native joint copies and detached connectors"),
                            ('assembly_inspect_bom', 'Read native BOM cells and quantity scope'),
                            ('assembly_parts_list', 'Count source identities across nested assemblies and export CSV'),
                            ('assembly_inspect_component', 'Inspect rigid/flexible instance paths and copied joints'),
                            ('assembly_configure_bom', 'Edit BOM options and columns'),
                            ('assembly_edit_bom_cells', 'Edit native custom BOM text'),
                            ("assembly_create_bom", "Create a bill of materials"),
                            ("assembly_export_bom_csv", "Export BOM spreadsheet to CSV"),
                            ("assembly_set_joint_state", "Suppress a joint or set angle/distance"),
                            ("assembly_set_joint_property", "Set an advanced native joint property"),
                            ("assembly_set_joint_limits", "Configure joint distance and angle limits"),
                            ("assembly_remove_joint", "Remove a joint"),
                            ("assembly_get_joint_types", "List native Assembly joint types"),
                            ("assembly_capabilities", "Report Assembly API capabilities"),
                            ("assembly_create_simulation", "Create an assembly motion simulation"),
                            ("assembly_inspect_simulation", "Inspect timing, motions, binding issues and frame freshness"),
                            ("assembly_edit_simulation", "Edit timing while preserving omitted values and expressions"),
                            ("assembly_edit_motion", "Edit a motion or explicitly repair its source/instance target"),
                            ("assembly_remove_motion", "Remove a native motion and invalidate frames"),
                            ("assembly_remove_simulation", "Remove a simulation and its owned motions"),
                            ('assembly_inspect_exploded_view', 'Inspect native steps and component instance paths'),
                            ('assembly_edit_exploded_step', 'Edit native step references and movement'),
                            ('assembly_reorder_exploded_steps', 'Reorder native exploded view steps'),
                            ('assembly_remove_exploded_step', 'Remove a native exploded step'),
                            ('assembly_remove_exploded_view', 'Remove native view and steps'),
                            ('assembly_exploded_frames', 'Calculate exploded animation frames without moving sources'),
                            ("assembly_create_exploded_view", "Create an exploded view container"),
                            ("assembly_export_exploded_shape", "Generate exploded geometry without moving components"),
                            ("assembly_add_exploded_step", "Add a component movement step"),
                            ("assembly_add_motion", "Add a kinematic simulation motion"),
                            ("assembly_run_simulation", "Generate assembly simulation frames"),
                            ("assembly_get_frame", "Display a simulation frame"),
                            ("assembly_flip_joint", "Flip a joint connector orientation"),
                            ("assembly_undo_solve", "Undo solver placement update"),
                            ("assembly_set_component_rigid", "Set subassembly link rigidity"),
                            ("assembly_inspect", "Inspect assembly members and joints"),
                            ("assembly_connection_graph", "Return component and joint connection graph"),
                            ("assembly_inspect_joint", "Inspect native joint properties"),
                            ("assembly_validate_references", "Validate joint connector references"),
                            ("assembly_get_freedom", "Report assembly solver component counts and estimated DOF"),
                            ("assembly_solver_diagnostics", "Diagnose solver errors and invalid joint references"),
                            ("assembly_component_status", "Inspect grounded state and placement"),
                            ("assembly_component_connectivity", "Check native ground connectivity"),
                            ("assembly_list_connectors", "List selectable joint reference elements"),
                            ("assembly_resolve_sub_element", "Resolve an Assembly subelement path"),
                            ("assembly_export_asmt", "Export checked ASMT constraints and report omitted components"),
                            ("assembly_import_asmt", "Probe/import an ASMT assembly file"),
                            ("assembly_get_global_placement", "Resolve object placement in assembly coordinates"),
                            ("assembly_set_property", "Set a writable Assembly object property"),
                            ("assembly_list_subobjects", "List Assembly subelement paths"),
                            ("assembly_set_element_visibility", "Set child element visibility"),
                            ("assembly_get_joint_connector", "Inspect a joint connector reference and placement"),
                            ("assembly_set_joint_connector", "Set joint connector detach state or placement"),
                            ("assembly_set_joint_references", "Reconnect or repair a native joint reference pair"),
                            ("assembly_set_joint_offset", "Set a geometry-attached connector offset and solve"),
                            ("assembly_simulation_status", "Check whether generated simulation frames are current"),
                            ("assembly_get_subobject_placement", "Resolve nested subobject placement"),
                        ]
                    ],
                },
                "cam_path": {
                    "description": "FreeCAD CAM/Path workbench",
                    "tools": [
                        {"name": n, "description": d} for n, d in [
                            ('cam_create_tool_library', 'Create a persistent native CAM library'),
                            ('cam_inspect_tool_library', 'Inspect library tool numbers and definitions'),
                            ('cam_edit_tool_library', 'Rename or edit numbered library members'),
                            ('cam_delete_tool_library', 'Delete a library retaining shared ToolBits'),
                            ('cam_export_tool_library', 'Export FCTL, CAMotics or LinuxCNC library'),
                            ('cam_import_tool_library', 'Import a native FCTL or CAMotics library'),
                            ('cam_add_library_tool_to_job', 'Attach numbered library ToolBit to a Job'),
                            ('cam_create_machine', 'Create a validated native CAM machine definition'),
                            ('cam_inspect_rotation_center', 'Inspect requested and actual native Path centers'),
                            ('cam_inspect_setup', 'Inspect machining model clones and stock placements'),
                            ('cam_inspect_setup_reference', 'Resolve clone/stock geometry points and directions'),
                            ('cam_align_setup', 'Align selected geometry with a machining axis'),
                            ('cam_set_setup_origin', 'Move a selected point to the machining origin'),
                            ('cam_center_setup_in_stock', 'Center combined model bounds inside explicit stock'),
                            ('cam_inspect_model_references', 'Inspect dependencies and required topology mapping'),
                            ('cam_replace_job_model', 'Replace a clone source and explicitly remap operation references'),
                            ('cam_set_operation_bases', 'Set validated multi-model operation Base references'),
                            ('cam_transform_setup', 'Transform Job models and stock and regenerate paths'),
                            ('cam_add_job_models', 'Add native model clones to a Job'),
                            ('cam_remove_job_model', 'Remove a Job model clone while preserving its source'),
                            ('cam_edit_machine', 'Edit machine limits with verified persistence'),
                            ('cam_import_machine', 'Import native machine assets with correct units'),
                            ('cam_export_machine', 'Export native machine assets to FCM JSON'),
                            ('cam_inspect_machine', 'Inspect machine limits'),
                            ('cam_delete_machine', 'Delete a machine definition'),
                            ('cam_evaluate_machine', 'Evaluate native machine torque at an RPM'),
                            ('cam_validate_job_machine', 'Validate Job controllers against machine limits'),
                            ("cam_create_job", "Create native Path Job"),
                            ("cam_add_tool_controller", "Create and attach ToolController and ToolBit asset"),
                            ("cam_add_operation", "Create Profile/Pocket/Drilling/Adaptive/Helix/Engrave/Surface operation"),
                            ("cam_generate_gcode", "Post-process Job to G-code"),
                            ("cam_get_post_processor_info", "Inspect native postprocessor arguments and units"),
                            ("cam_set_setup_sheet", "Edit native SetupSheet defaults and recompute"),
                            ("cam_generate_setup_report", "Generate native CAM setup report with structured warnings"),
                            ("cam_inspect_job", "Inspect CAM Job"),
                            ("cam_list_post_processors", "List installed post processors"),
                            ("cam_list_tool_library", "List native CAM ToolBit assets"),
                            ("cam_set_job_settings", "Configure Job post processor and output"),
                            ("cam_set_job_property", "Set an advanced native Job property"),
                            ("cam_set_operation_state", "Activate or deactivate a Path operation"),
                            ("cam_set_fixture", "Assign Job work coordinate codes such as G54/G55"),
                            ("cam_get_setup_sheet", "Inspect native Job setup sheet"),
                            ("cam_get_toolpath", "Inspect generated toolpath commands"),
                            ("cam_create_custom_path", "Create a native custom toolpath"),
                            ("cam_set_tool_controller", "Update tool controller settings"),
                            ("cam_export_tool_controller", "Export ToolController and ToolBit data"),
                            ("cam_set_stock", "Create or update Job stock"),
                            ("cam_get_operation_schema", "Inspect native operation properties"),
                            ("cam_recompute_operation", "Recompute one operation"),
                            ("cam_set_operation_base", "Set operation geometry and subelements"),
                            ("cam_add_dressup", "Add a native Path dressup"),
                            ("cam_list_dressups", "List installed Path dressups"),
                            ("cam_set_operation_parameters", "Update native operation parameters"),
                            ("cam_list_operations", "List installed Path operations"),
                            ("cam_capabilities", "Report CAM API capabilities"),
                            ("cam_simulate_toolpath", "Compute toolpath statistics"),
                            ("cam_simulate_stock", "Run native stock removal simulation when GUI is available"),
                            ("cam_validate_job", "Validate Job before posting"),
                            ("cam_job_statistics", "Aggregate CAM path length and command statistics"),
                            ("cam_validate_operation_parameters", "Validate parameters against a Path operation"),
                            ("cam_remove_operation", "Remove a Path operation and dependent dressups"),
                            ("cam_remove_tool_controller", "Remove a ToolController from its Job"),
                            ("cam_set_tool_controller_property", "Set an advanced ToolController property"),
                            ("cam_reorder_operations", "Reorder top-level Job operations"),
                            ("cam_set_center_of_rotation", "Set the Job toolpath rotation center"),
                            ("cam_create_job_from_template", "Create a Job from a native template"),
                            ("cam_export_job_template", "Export Job settings as a native template"),
                            ("cam_import_toolbit", "Import a ToolBit file and attach it to a controller"),
                            ("cam_export_toolbit", "Export a native FCTB ToolBit file"),
                        ]
                    ],
                },
                "surface": {
                    "description": "Part and Surface modeling operations",
                    "tools": [
                        {"name": n, "description": d} for n, d in [
                            ('surface_create_cut', 'Create a native linked Surface::Cut feature'),
                            ('surface_tessellate', 'Mesh native faces with OCC deflection controls'),
                            ('surface_from_mesh', 'Convert mesh triangles into a faceted Part shape'),
                            ('surface_set_parameter_range', 'Rescale spline knots and trim UV without changing geometry'),
                            ("surface_create_bspline", "Create exact NURBS from control poles, knots and weights"),
                            ("surface_interpolate_points", "Interpolate sampled points as a BSpline surface"),
                            ("surface_create_bezier", "Create Bezier surface"),
                            ("surface_ruled", "Create ruled surface"),
                            ("surface_fill_boundary", "Fill a boundary"),
                            ("surface_loft", "Loft through sections"),
                            ("surface_sweep", "Sweep a profile"),
                            ("surface_offset", "Offset a surface"),
                            ("surface_trim", "Trim with a tool shape"),
                            ("surface_sew", "Sew faces into a shell"),
                            ("surface_to_solid", "Convert a closed shell to solid"),
                            ("surface_inspect", "Inspect surface topology and bounds"),
                            ("surface_reverse", "Reverse surface orientation"),
                            ("surface_to_nurbs", "Convert surface to NURBS"),
                            ("surface_extract_face", "Extract a single face as a surface"),
                            ("surface_replace_faces", "Sew edited faces into a copied shell/solid and validate closure"),
                            ("surface_intersection", "Intersect two surface or solid shapes"),
                            ("surface_boundary_info", "Inspect surface boundary wires"),
                            ("surface_surface_info", "Inspect native surface type and parameters"),
                            ("surface_evaluate", "Evaluate a surface at U/V parameters"),
                            ("surface_extract_isocurve", "Extract a U/V isoparametric curve"),
                            ("surface_segment", "Trim by surface parameter domain"),
                            ("surface_extend", "Create native Surface::Extend using fractional U/V extension"),
                            ("surface_check_continuity", "Sample G0/G1/G2 between selected face boundaries"),
                            ("surface_extrude", "Extrude a surface or wire"),
                            ("surface_revolve", "Revolve a surface around an axis"),
                            ("surface_project", "Project edges onto a surface"),
                            ("surface_transform", "Transform a surface copy"),
                            ("surface_edit_pole", "Move a native surface control pole"),
                            ("surface_edit_weight", "Set a rational surface pole weight"),
                            ("surface_normal", "Evaluate an oriented face normal with explicit UV space"),
                            ("surface_project_point", "Project onto the underlying surface and report trimmed-domain membership"),
                            ("surface_exchange_uv", "Swap native U/V surface directions"),
                            ("surface_reparameterize", "Refit a selected surface with sampled error checks"),
                            ("surface_tangent", "Evaluate U/V surface tangents"),
                            ("surface_derivative", "Evaluate a surface derivative"),
                            ("surface_get_pole", "Read a surface control pole and weight"),
                            ("surface_set_knot", "Edit a BSpline knot"),
                            ("surface_set_periodic", "Toggle BSpline U/V periodicity"),
                            ("surface_insert_knot", "Insert a BSpline U/V knot"),
                            ("surface_remove_knot", "Remove a BSpline U/V knot"),
                            ("surface_increase_degree", "Elevate BSpline U/V degree"),
                            ("surface_edit_control_net", "Apply a batch of pole and weight edits atomically"),
                            ("surface_create_sections", "Create a parametric surface through section edges"),
                            ("surface_create_filling", "Create native C0/G1/G2 constrained filling"),
                            ("surface_blend_curve", "Join edges with a parametric continuity-controlled blend"),
                            ("surface_control_net", "Read poles, weights, knots and multiplicities"),
                            ("surface_trim_parameters", "Trim a copy to a parameter domain"),
                            ("surface_curvature", "Evaluate native surface curvature"),
                            ("surface_capabilities", "Probe native surface APIs and feature schemas"),
                        ]
                    ],
                },
                "patterns": {
