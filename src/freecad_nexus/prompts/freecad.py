"""FreeCAD Nexus prompts for common CAD tasks.
...
def register_prompts(mcp: Any, get_bridge: Any) -> None:  # noqa: ARG001
    """Register FreeCAD prompts with the FreeCAD Nexus.
...
You are connected to the FreeCAD Nexus Server. Follow these guidelines for reliable operations.
...
Document changes use transactions where supported:
- Use `undo()` to revert recorded document edits
- Use `redo()` to redo after undo
- Use `get_undo_redo_status()` to see available undo steps
- File exports and external asset changes are not reverted by document Undo.
- Finish any pending interactive transaction before calling specialized mutation tools.
...
- Inspect the returned success/error and verify the resulting geometry or toolpath.
...
Read native capability reports before using version-dependent workbenches.
Assembly, CAM and Surface integration has verified workflows but is not complete
coverage of every installed API, operation or parameter combination.
...
| Undo recorded document edits | `undo()` |
...
Some Assembly simulation/exploded-view classes, CAM dressups and stock simulation
also require GUI mode in the tested build. Use the corresponding capability
reports and returned errors; registration does not establish runtime availability.

## Specialized Workbench Conventions

- Surface evaluation and isocurves default to `parameter_space="normalized"`.
  Use `"native"` explicitly for native UV. Surface trim/segment default to native.
  Face indexes are zero-based. Returned UV/derivatives are native; `inside_face`
  reports whether evaluation lies within the trimmed face, including holes.
- Spline editing uses zero-based face_index and one-based pole/knot indexes.
  Pole coordinates are in document space. Edits return a copied selected face
  with its UV trim loops/holes; other faces and the source remain unchanged.
  Shared seams and shell closure are preserved; splitting one side of an
  existing seam fails. Results expose seam_edges and closed_shell.
- `surface_set_parameter_range` changes knot bounds without changing geometry.
  `surface_reparameterize` is a sampled refit with measured, unproven error bounds.
- `surface_create_cut` retains native links. Mesh conversion produces triangle
  faces, not a fitted smooth surface.
- `surface_check_continuity` samples one complete boundary pair for G0/G1/G2.
  For a second single-face object set `other_face_index=0`. A passing sample
  result is not proof between sample points. Singularities may be inconclusive.
- CAM numeric feeds use mm/min, spindle speeds RPM. `path_ready` requires
  machining/probing commands but does not verify correct machining or collisions.
  Path statistics support G17/G18/G19 circular/helical motion, signed R and
  absolute/incremental IJK centers. G81/G82/G85 expand with LinuxCNC RS274
  G98/G99, sticky depth/R and L-repeat semantics. G82 dwell seconds are separate
  from path length. Cancel cycles before changing plane, units or distance mode.
  Unsupported peck/tapping cycles, axes or P turns return
  incomplete statistics; no full machine cycle time is claimed.
  `surface_replace_faces` reinserts edited faces into a copied shell/solid.
  Select zero-based source/replacement face indexes and fit shared boundaries
  before sewing. Shell closure and solid structure must remain valid; face
  numbering can change. The result is a snapshot with source objects retained.
- CAM setup tools select Job model clones, not design sources. Inspect setup
  references before axis alignment or origin translation. Model replacement
  requires explicit mapping of every referenced subelement; inspect model
  references first. Source-selected Base creation resolves to a unique clone.
- Assembly solver diagnostics distinguish invalid references from conflicts;
  nominal DOF is an estimate, not exact constraint rank.
  Flexible instance joint parameters and detached connectors are synchronized
  before MCP solve/simulation; `assembly_instance_sync_status` audits differences.
  `assembly_sync_instances(solve=False)` preserves existing component poses.
  Inspect `motion_bindings` separately from parameter `current`: tracked Motion
  targets follow source joint identity across native copy reorder/recreation.
  Suppressed/deleted sources prevent generation. Older/UI-created copy motions
  need explicit adoption with `assembly_edit_motion(joint_name=...)`.
  `assembly_inspect_simulation` lists every configuration issue. Repair motions
  individually, then run only when the whole configuration is valid. Omitted
  timing properties/expressions are retained by `assembly_edit_simulation`.
  Formula/geometry edits invalidate frames; display material changes do not.
  Stale frame diagnostics name changed objects/properties. Angular formulas
  use radians, linear formulas mm, and time seconds.
  ASMT export validates native constraint names/marker links and includes deep
  instance joints. Check `omitted_components`: disconnected parts may be absent.
  The file contains no CAD shapes, measured mass/inertia or Simulation motions.
...
"""
    ...
    async def troubleshooting() -> str:
        """General troubleshooting guide for FreeCAD Nexus.

        Returns:
            Troubleshooting guidance.
        """
        return """# FreeCAD Nexus Troubleshooting Guide
