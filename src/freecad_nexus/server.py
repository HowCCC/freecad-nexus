"""FreeCAD Nexus Server - Main entry point.

This module provides the main FreeCAD Nexus implementation for FreeCAD
...
Example:
    ...
    With environment variables::
        ...
        $ FREECAD_MODE=socket FREECAD_SOCKET_HOST=localhost freecad-nexus
...
from freecad_nexus.config import FreecadMode, TransportType, get_config

if TYPE_CHECKING:
    from freecad_nexus.bridge.base import FreecadBridge
...
def get_instance_id() -> str:
    """Get the unique instance ID for this FreeCAD Nexus process.
...
async def lifespan(_server: FastMCP) -> AsyncIterator[None]:
    ...
    if config.mode == FreecadMode.EMBEDDED:
        from freecad_nexus.bridge.embedded import EmbeddedBridge
        ...
        from freecad_nexus.bridge.xmlrpc import XmlRpcBridge
...
# Create the FreeCAD Nexus instance with lifespan
mcp = FastMCP(
    name="freecad-nexus",
    lifespan=lifespan,
)
...
def register_all_components() -> None:
    ...
    # Register tools
    from freecad_nexus.tools import register_all_tools

    register_all_tools(mcp, get_bridge)

    # Register resources
    from freecad_nexus.resources import register_resources

    register_resources(mcp, get_bridge)

    # Register prompts
    from freecad_nexus.prompts import register_prompts
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        prog="freecad-nexus",
        description="FreeCAD Nexus Server - Connect AI assistants to FreeCAD",
...
Examples:
  ...
  # Use socket mode
  FREECAD_MODE=socket freecad-nexus

  # Use HTTP transport for remote access
  FREECAD_TRANSPORT=http FREECAD_HTTP_PORT=8080 freecad-nexus

  # Connect to remote FreeCAD instance
  FREECAD_SOCKET_HOST=192.168.1.100 freecad-nexus
...
def main() -> None:
    ...
    if args.version:
        try:
            ...
            ver = version("freecad-nexus")
        except Exception:
            ver = "unknown"
        print(f"freecad-nexus {ver}")
    ...
    logger.info("Starting FreeCAD Nexus Server")
    ...
    if config.transport == TransportType.HTTP:
        ...
        logger.info(
            "Tip: Use 'freecad-nexus --check' to test FreeCAD connection directly"
        )
        mcp.run()
